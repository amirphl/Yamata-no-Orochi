...
// RubikaStatusResult stores provider status metrics for a Rubika message tracking row.
type RubikaStatusResult struct {
	ID                    uint            `gorm:"primaryKey" json:"id"`
	JobID                 uint            `gorm:"column:job_id;index:idx_rubika_status_results_job_id;not null" json:"job_id"`
	ProcessedCampaignID   uint            `gorm:"column:processed_campaign_id;index:idx_rubika_status_results_processed_campaign_id;uniqueIndex:idx_rubika_status_results_processed_campaign_tracking,priority:1;not null" json:"processed_campaign_id"`
	TrackingID            string          `gorm:"column:tracking_id;type:text;index:idx_rubika_status_results_tracking_id;uniqueIndex:idx_rubika_status_results_processed_campaign_tracking,priority:2;not null" json:"tracking_id"`
...
func (RubikaStatusResult) TableName() string { return "rubika_status_results" }
