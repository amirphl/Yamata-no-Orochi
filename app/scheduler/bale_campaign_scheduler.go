...
func (s *BaleCampaignScheduler) fetchBaleAudiencePhones(
	...
	s.logger.Printf("fetchBaleAudiencePhones start: campaign_id=%d customer_id=%d num_audiences=%d tags_length=%d correlation_id=%s", c.ID, c.CustomerID, numAudiences, len(executionTags), correlationID)
	...
	s.logger.Printf("fetchBaleAudiencePhones tags resolved: campaign_id=%d requested=%d resolved=%d", c.ID, len(executionTags), len(tagIDs))
...
func (s *BaleCampaignScheduler) selectBaleTagAudiences(
	...
	s.logger.Printf("selectBaleTagAudiences candidates: campaign_id=%d count=%d", campaignID, len(candidates))
...
func (s *BaleCampaignScheduler) fetchBaleAudiencePhonesByBundle(
	...
	for i, tag := range executionTags {
		tagID, err := strconv.ParseUint(tag, 10, 32)
		if err != nil {
			s.logger.Printf("fetchBaleAudiencePhonesByBundle tag parse failed: campaign_id=%d tag=%q err=%v", c.ID, tag, err)
			return nil, err
		}
		toExtract[i] = uint(tagID)
	}
	tags, err := s.tagRepo.ListByIDs(ctx, toExtract)
	if err != nil {
		s.logger.Printf("fetchBaleAudiencePhonesByBundle tags lookup failed: campaign_id=%d err=%v", c.ID, err)
		return nil, err
	}
	tagIDs := make(pq.Int32Array, len(tags))
	for i, tag := range tags {
	...
	if err != nil {
		s.logger.Printf("fetchBaleAudiencePhonesByBundle latest bundle selection failed: campaign_id=%d bundle_id=%d err=%v", c.ID, bundleID, err)
		return nil, err
	}
	if bundleSel != nil {
		exclude = bundleSel.IDs
		s.logger.Printf("fetchBaleAudiencePhonesByBundle bundle selection hit: campaign_id=%d bundle_id=%d prior_ids=%d", c.ID, bundleID, len(bundleSel.IDs))
	} else {
	...
	if err != nil {
		s.logger.Printf("fetchBaleAudiencePhonesByBundle selection save failed: campaign_id=%d bundle_id=%d err=%v", c.ID, bundleID, err)
		return nil, err
	}
