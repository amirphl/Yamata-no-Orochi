...
func TestDispatchPendingSplusCampaignsSerializesSameBundle(t *testing.T) {
	t.Parallel()
	...
	s.dispatchPendingSplusCampaigns(parent, "token-1", pending, func(ctx context.Context, token string, c dto.BotGetCampaignResponse) error {
		if token != "token-1" {
