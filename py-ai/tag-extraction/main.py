...
if stats_df is not None:
    ...
    if not missing_stats:
        ...
        stats["white_users"] = (
            pd.to_numeric(stats["white_users"], errors="coerce").fillna(0).astype(int)
        )
        ...
        available = stats[["level1", "level2", "level3", "white_users"]]
...
if available is not None:
    ...
    out = out.rename(columns={"white_users": "available_audience"})
...
if defs is not None:
    ...
    if "شناسه فارسی" in keys:
        ...
        out["level2_inclusion"] = out["level2"].map(
            lambda v: lookup_meta(v, "inclusion")
        )
        out["level2_exclusion"] = out["level2"].map(
            lambda v: lookup_meta(v, "exclusion")
        )
    else:
