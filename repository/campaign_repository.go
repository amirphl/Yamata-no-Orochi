...
func excludeAutomatedClickTraffic(db *gorm.DB, ipExpr, userAgentExpr string) *gorm.DB {
	return db.
		Where(fmt.Sprintf("COALESCE(%s, '') !~ ?", ipExpr), "^(66\\.249\\.|74\\.125\\.)").
		Where(fmt.Sprintf(`NOT (
			COALESCE(%s, '') ~ 'Chrome'
			AND COALESCE(%s, '') !~ '(Edg|OPR|Opera)'
			AND (
				COALESCE(%s, '') ~* 'X11; Linux|Linux'
				AND COALESCE(%s, '') !~* 'Android|Windows NT|Mac OS X|Macintosh|iPhone|iPad|iPod'
			)
		)`, userAgentExpr, userAgentExpr, userAgentExpr, userAgentExpr))
}
...
func (r *CampaignRepositoryImpl) AggregateClickCountsByCampaignIDs(ctx context.Context, campaignIDs []uint) (map[uint]int64, error) {
	...
	var rows []row
	db := excludeAutomatedClickTraffic(r.getDB(ctx), "ip", "user_agent")
	if err := db.Table("short_link_clicks").
		Select("campaign_id, COUNT(DISTINCT uid) AS clicks").
		Where("campaign_id IN ?", campaignIDs).
		Group("campaign_id").
