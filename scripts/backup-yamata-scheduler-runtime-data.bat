...
REM Canonical second selective backup. audience_profiles and sent_rubika_messages are excluded.
...
set "DUMP_FILE=%OUTDIR%\scheduler_runtime_%TS%.dump"
if exist "%DUMP_FILE%" (
...
"%PGBIN%\pg_dump.exe" -h "%PGHOST%" -p "%PGPORT%" -U "%PGUSER%" -d "%PGDB%" ^
  ...
  -t public.sequence_counters ^
  -t public.src_layer_all_stats ^
  -t public.src_reference ^
  --file="%DUMP_FILE%"
