...
class Database:
    ...
    async def acknowledge(self, through_click_id: int) -> int:
        maximum = int(await self._pool().fetchval("SELECT COALESCE(MAX(click_id), 0) FROM clicks"))
        if through_click_id > maximum:
            raise ValueError("through_click_id exceeds the highest persisted click_id")
        return int(
            await self._pool().fetchval(
                """
                UPDATE click_acknowledgements
                SET through_click_id = GREATEST(through_click_id, $1),
    ...
    async def purge_acknowledged(self, retention_days: int) -> int:
        ...
        result = await self._pool().execute(
            """
            DELETE FROM clicks
            WHERE click_id <= (SELECT through_click_id FROM click_acknowledgements WHERE singleton = TRUE)
              AND clicked_at < $1
            """,
            cutoff,
        )
