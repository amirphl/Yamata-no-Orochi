...
def test_postgres_mapping_click_cursor_and_idempotency() -> None:
    async def scenario() -> None:
        ...
        try:
            ...
            refreshed = {**mapping, "campaign_id": 124, "source_link_id": 88}
            created, existing, _ = await database.upload_links([refreshed])
            ...
            assert link["campaign_id"] == 124
            assert link["source_link_id"] == 88
            first = {
            ...
            async with database.pool.acquire() as connection:
                acknowledged = await connection.fetchval(
                    "SELECT COUNT(*) FROM clicks WHERE acknowledged_at IS NOT NULL"
                )
                assert acknowledged == 2
                await connection.execute(
                    "UPDATE clicks SET acknowledged_at = CURRENT_TIMESTAMP - INTERVAL '8 days'"
                )
            assert await database.purge_acknowledged(7) == 2
            # A lost acknowledgement response must remain idempotently retryable
            # even after the acknowledged rows have aged out and been purged.
            assert await database.acknowledge(page_two[0]["click_id"]) == page_two[0]["click_id"]
        finally:
