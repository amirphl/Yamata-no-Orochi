-- Replace disposable smart-tag UUID identifiers with BIGSERIAL identifiers.

BEGIN;

-- These views depend on the identifier columns replaced below.
...
-- Evaluation records are immutable and UUID values cannot be meaningfully
-- translated after their columns are dropped. The caller explicitly chose to
-- discard this feature's data, so clear all related tables in one operation.
TRUNCATE TABLE
    bundle_tag_scores,
...
-- Remove foreign keys before replacing their columns.
...
-- PostgreSQL drops indexes involving a removed column, but naming them here
-- makes their recreation below explicit and keeps the migration deterministic.
...
-- BIGSERIAL creates a signed 64-bit column backed by an owned sequence.
ALTER TABLE bundle_tag_evaluation_runs
    ADD COLUMN id BIGSERIAL PRIMARY KEY;

ALTER TABLE bundle_tag_persona_analysis_attempts
    ADD COLUMN id BIGSERIAL PRIMARY KEY,
    ADD COLUMN evaluation_run_id BIGINT NOT NULL;
...
ALTER TABLE bundle_tag_scores
    ADD COLUMN evaluation_run_id BIGINT NOT NULL,
    ADD COLUMN batch_id BIGINT NOT NULL,
    ADD COLUMN batch_attempt_id BIGINT NOT NULL;
