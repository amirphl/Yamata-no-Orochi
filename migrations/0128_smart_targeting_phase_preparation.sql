...
WITH ordered AS (
    -- Existing rows predate explicit ordering. Their monotonically increasing
    -- row IDs are the closest persisted representation of insertion order.
    SELECT id, ROW_NUMBER() OVER (PARTITION BY campaign_id ORDER BY id) - 1 AS position
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'campaign_selected_tags'::regclass
          AND conname = 'campaign_selected_tags_selection_order_nonnegative'
    ) THEN
        ALTER TABLE campaign_selected_tags
            ADD CONSTRAINT campaign_selected_tags_selection_order_nonnegative
...
WITH ordered AS (
    -- CreateInBatches inserted members in scheduler selection/send order, so
    -- preserve that historical sequence instead of sorting by audience ID.
    SELECT id, ROW_NUMBER() OVER (PARTITION BY selection_id ORDER BY id) - 1 AS position
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'bundle_audience_selection_members'::regclass
          AND conname = 'bundle_aud_sel_member_selection_order_nonnegative'
    ) THEN
        ALTER TABLE bundle_audience_selection_members
            ADD CONSTRAINT bundle_aud_sel_member_selection_order_nonnegative
            CHECK (selection_order >= 0);
    END IF;
END $$;
